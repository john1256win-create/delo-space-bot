...
def build_dev_stats_block(conn: sqlite3.Connection, rel: dict,
    ...
    Затем иерархия (требования пользователя, финал 2026-09-22):
      • 🏋 ***Разработчик*** — N задач     (курсив + полужирный, 🏋 перед ФИО)
        *Статус*                  (курсив, НЕ полужирный; приоритет STATUS_ORDER)
        1) номер — название — разр. план / факт в релизе [стрелка] Всего X ч
           (номер задачи БЕЗ полужирного)
    ...
    for name in names:
        ...
        # Разработчик — ФИО КУРСИВ + ПОЛУЖИРНЫЙ (***ФИО***), значок 🏋 перед ФИО
        # (требование пользователя, 2026-09-22).
        block = [
        ...
        counter = 0
        for st in statuses:
            # Статус — КУРСИВ, НЕ полужирный (требование пользователя, 2026-09-22).
            block.append(f"*{st}*")
            for t in sorted(devs[name][st], key=lambda x: x["request_code"]):
                counter += 1
                plan, fact = t.get("development_plan"), t.get("fact_dev")
                # Номер задачи — БЕЗ полужирного (требование пользователя, 2026-09-22).
                line = (
