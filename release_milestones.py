...
# Порядок вывода статусов задач (как в фильтре Web UI, константа STATUS_ORDER
# в frontend/src/pages/ReleaseTasksTable.tsx). Статусы, отсутствующие в списке,
# выводятся в конце в произвольном порядке.
STATUS_ORDER = [
    "Зарегистрирована",
    "В ожидании, линия 1",
    "В работе (1 линия)",
    "В ожидании, линия 2",
    "В работе (2 линия)",
    "Ожидание уточнения от автора",
    "Ожидает решения/события",
    "На согласовании",
    "Не согласована",
    "Согласована",
    "Ожидает консультации программиста",
    "Ожидает подготовки ТЗ",
    "Подготовка ТЗ",
    "Консультация программиста",
    "Ожидает проверки ТЗ",
    "На проверке ТЗ",
    "Установка ответственного программиста для консультации",
    "Готова к включению в релиз",
    "Ожидает разработки",
    "В разработке",
    "Ожидает работу QA",
    "В работе QA",
    "Ожидает проверки методологом",
    "На проверке у методологов",
    "На тестировании у ключевых пользователей",
    "На загрузке расширения",
    "Ожидает обновления системы",
    "Закрыта",
    "Не актуальна",
    "Отменена",
    "Закупка выполнена",
    "В снабжении",
    "Ожидает проверки 1 линией",
    "На проверке у 1 линии",
]
...
def format_milestone_message(release: str, milestone_name: str, counter: Counter,
    ...
    # Сводка по статусам задач (в порядке STATUS_ORDER из фильтра Web UI)
    if counter:
        parts = []
        # 1) известные статусы — в заданном порядке
        for status in STATUS_ORDER:
            count = counter.get(status, 0)
            if count > 0:
                parts.append(f"{status}: {count}")
        # 2) неизвестные (не в списке) — в конце, по убыванию
