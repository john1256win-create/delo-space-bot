...
# Группировка статусов задач для сообщения
STATUS_GROUPS = {
    "В работе": ["В работе (1 линия)", "В работе (2 линия)", "В разработке",
                 "На тестировании у ключевых пользователей"],
    "На проверке": ["Ожидает проверки методологом", "Ожидает проверки ТЗ",
                    "На проверке у методологов", "На согласовании", "Согласована",
                    "Ожидает разработки", "Ожидает работу QA", "Ожидает подготовки ТЗ"],
    "Новые": ["Зарегистрирована", "Готова к включению в релиз"],
    "Ожидание": ["В ожидании, линия 1", "В ожидании, линия 2",
                 "Ожидает обновления системы", "Ожидание уточнения от автора",
                 "Ожидает решения/события", "Ожидает консультации программиста"],
    "Закрыта": ["Закрыта"],
}
...
def group_task_statuses(tasks: list[dict]) -> Counter:
    """Считает задачи по группам статусов."""
    counter = Counter()
    status_col = None
    ...
    for t in tasks:
        ...
        for group, statuses in STATUS_GROUPS.items():
            if st in statuses:
                ...
                grouped = True
                break
        if not grouped and st:
            counter[st] += 1  # незнакомая группа — покажем как есть
    return counter
